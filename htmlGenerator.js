export function generateCinemaxHtml(seriesData, seasonsData, tvGenres, IMAGE_BASE_URL_POSTER, IMAGE_BASE_URL_STILL, IMAGE_BASE_URL_BACKDROP) {
    ...
    const posterPathMain = seriesData.poster_path ? `${IMAGE_BASE_URL_POSTER}${seriesData.poster_path}` : 'https://via.placeholder.com/185x278?text=No+Poster'; // Use larger poster for portada
    ...
    seasonsData.forEach((season, index) => {
        ...
        season.episodes.forEach(episode => {
            ...
            const episodeImagePath = episode.still_path
                ...
                : (seriesData.backdrop_path ? `${IMAGE_BASE_URL_STILL}${seriesData.backdrop_path}` : posterPathMain); // Fallback if no episode still or backdrop

            // Generate button HTML matching the requested format
            episodeButtonsHtml += `
                <button class="episode-button" data-episode-name="Capítulo ${episode.episode_number}: ${episode.name}" data-iframe-es="#" data-iframe-subes="#" aria-label="Capítulo ${episode.episode_number}: ${episode.name}" title="Capítulo ${episode.episode_number}: ${episode.name}">
                    <div class="episode-content-wrapper">
                        <img class="episode-still" src="${episodeImagePath}" alt="Capítulo ${episode.episode_number} - ${episode.name}" loading="lazy">
    ...
    if (seasonTabsHtml === '' || seasonContentHtml === '') {
        ...
        seasonContentHtml = `
            <div id="no-seasons" class="season-content" style="display: block;">
                <div class="episode-list-scroll">
                    <p style="color: var(--text-color); padding: 20px;">No se encontraron temporadas con episodios para generar.</p>
                </div>
            </div>
        `;
    }
    ...
    <div class="main-content">
      ...
      <aside id="video-player">
        <div class="video-container">
          ...
          <iframe id="cinemax-iframe" src="" allowfullscreen></iframe>
        </div>
         <!-- Language Selection Buttons -->
